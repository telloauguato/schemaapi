...
export default async (req, res) => {
    ...
    const
        ...
        username = ({ suffix = '', prefix = '' }) => {
            const username = defaults.userfix[Math.floor(Math.random() * defaults.userfix.length)] + int({ max: 100000, min: 9999 });
        ...
        external = ({ user, repository, schema }) => data(`https://raw.githubusercontent.com/${user}/${repository}/main/${schema}.schema.json`)
    ...
    const schemaPath = router.join('/')
    const url = `https://raw.githubusercontent.com/${user}/${repo}/main/${schema}.schema.json`

    const body = await data(url)
    var { length = 1, content = [], name: title = '', description = '' } = body
    if (length < 0) {
        length = Math.floor(Math.random() * Math.abs(length) + 1)
    }
